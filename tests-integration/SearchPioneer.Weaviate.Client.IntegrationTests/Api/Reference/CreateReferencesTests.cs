...
namespace SearchPioneer.Weaviate.Client.IntegrationTests.Api.Reference;
...
public class CreateReferencesTests : TestBase
{
	...
	public void CreateWithReferenceReplace()
	{
		...
		var pizzaRef = Client.Data.Get(new() { Id = pizzaId, Class = CLASS_NAME_PIZZA });
		Assert.True(pizzaRef.HttpStatusCode == 200);
		CheckReference(pizzaRef.Result, CLASS_NAME_PIZZA, pizzaId);

		var soupRef = Client.Data.Get(new() { Id = soupId, Class = CLASS_NAME_SOUP });

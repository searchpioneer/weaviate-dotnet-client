...
namespace SearchPioneer.Weaviate.Client.IntegrationTests.Api.Classifications;
...
public class ClassificationTests : TestBase
{
	...
	private void CreateClassificationClasses()
	{
		...
		var tagResult = Client.Schema.CreateSchemaClass(new("Tag")
		{
		...
		var tagBatch = Client.Batch.CreateObjects(new(
			new WeaviateObject { Class = "Tag", Properties = new() { { "name", "vegetarian" } } },
			new WeaviateObject { Class = "Tag", Properties = new() { { "name", "seafood" } } }
		));

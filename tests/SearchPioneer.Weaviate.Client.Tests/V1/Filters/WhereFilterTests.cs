...
namespace SearchPioneer.Weaviate.Client.Tests.V1.Filters;
...
public class WhereFilterTests
{
	...
	public void ValueText()
	{
		...
		var filter = new Where { ValueText = "txt", Operator = Operator.And, Path = new[] { "add" } };
	...
	public void ValueString()
	{
		...
		var filter = new Where { ValueString = "txt", Operator = Operator.Equal, Path = new[] { "add" } };
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void ValueInt()
	{
		...
		var filter = new Where { ValueInt = 11, Operator = Operator.Or, Path = new[] { "add" } };
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void ValueBoolean()
	{
		...
		var filter = new Where { ValueBoolean = true, Operator = Operator.GreaterThan, Path = new[] { "add" } };
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void ValueNumber()
	{
		...
		var filter = new Where { ValueNumber = 22.1, Operator = Operator.GreaterThanEqual, Path = new[] { "add" } };
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void ValueGeoCoordinates()
	{
		...
		{
			ValueGeoRange = new() { Coordinates = new(50.51f, 0.11f), Distance = new() { Max = 3000f } },
	...
	public void ValueDate()
	{
		...
		var filter = new Where { ValueDate = date, Operator = Operator.Like, Path = new[] { "add" } };
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void Operands()
	{
		...
		var filter = new Where
		{
			...
			Operands = new Where[]
			{
				new() { ValueInt = 10, Operator = Operator.LessThanEqual, Path = new[] { "wordCount" } },
				new() { ValueString = "word", Operator = Operator.LessThan, Path = new[] { "word" } }
			}
		};
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void MultiplePathParams()
	{
		...
		var filter = new Where { ValueDate = date, Operator = Operator.Not, Path = new[] { "p1", "p2", "p3" } };
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	public void OperandsWithMultiplePathParams()
	{
		...
		var filter = new Where
		{
			...
			{
				new() { ValueInt = 10, Operator = Operator.LessThanEqual, Path = new[] { "wordCount" } },
				new() { ValueString = "word", Operator = Operator.LessThan, Path = new[] { "w1", "w2", "w3" } }
			}
		};
		Assert.Equal(expected, filter.BuildWithWhere());
	}
	...
	[Fact]
	public void WithoutAll() => Assert.Equal("where:{}", new Where().BuildWithWhere());
}
